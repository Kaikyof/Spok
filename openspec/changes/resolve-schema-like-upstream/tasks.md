...
- [x] 1.4 Перевести `_taskFilesOf`, `_changeDocs`, `argumentValues`, `loadDocTree` и `change_screen._artifacts` на схему change'а; `loadSchema()` оставить для экрана распознавания с источником; проверить эталоном «до», что на avtoto и avelacom стеки, файлы и артефакты не изменились
...
- [x] 3.5 Обновить `tool/smoke.dart` (источник схемы — реальное значение) и прогнать `tool/compare_smoke.sh` на avtoto и avelacom — разница только в новых строках вывода
