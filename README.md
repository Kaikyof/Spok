...
- **Передача** — четыре шага: готовность с блокерами, сборки, получатели,
  предпросмотр сообщения и побочных эффектов (отправка — следующий этап).
- **Сессии** — агентные диалоги: запуск Claude Code headless-процессом
  в репозитории платформы (`claude -p --output-format stream-json`),
  транскрипт с действиями по ходу, выбор модели, остановка.
