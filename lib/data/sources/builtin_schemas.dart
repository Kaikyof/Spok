/// Встроенные схемы оригинального OpenSpec.
///
/// Схема `spec-driven` живёт в npm-пакете upstream, а не в проекте, поэтому
/// у проекта после `openspec init` в `openspec/schemas/` пусто. Копия нужна
/// приложению, чтобы открывать такие проекты без установленного CLI.
///
/// Текст — константа, а не asset: слой данных читается и из `dart run
/// tool/smoke.dart`, где `rootBundle` недоступен. Источник правды —
/// `assets/openspec/schemas/spec-driven/schema.yaml`; тест
/// `builtin_schema_test.dart` сверяет константу с файлом, чтобы они не
/// разошлись.
library;

/// Версия пакета upstream, с которой снята копия.
const builtinSchemaUpstreamVersion = '1.13.2';

/// Имя схемы по умолчанию — та же, что подставляет upstream без конфига.
const defaultSchemaName = 'spec-driven';

/// Встроенные схемы: имя → текст `schema.yaml`.
const builtinSchemas = <String, String>{
  defaultSchemaName: builtinSpecDrivenSchema,
};

/// `schemas/spec-driven/schema.yaml` из upstream 1.13.2, дословно.
const builtinSpecDrivenSchema = r'''
# Копия встроенной схемы оригинального OpenSpec — @fission-ai/openspec 1.13.2,
# schemas/spec-driven/schema.yaml (коммит db23097). Не редактировать: при
# обновлении upstream заменить целиком и обновить версию здесь и в
# lib/data/sources/builtin_schemas.dart (тест сверяет их между собой).
name: spec-driven
version: 1
